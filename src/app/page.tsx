  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white font-sans antialiased">
      ...
      <section className="border-b border-white/10">
        <div className="max-w-6xl mx-auto px-6 py-20 md:py-28">
          <h1 className="text-4xl md:text-6xl font-bold tracking-tight text-white mb-6 leading-tight">
            Open Source<br />
            <span className="text-neutral-500">Brand Logo Library</span>
          </h1>
          ...
          <div className="flex gap-12">
            {[
              ...
              <div key={s.label}>
                <div className="text-3xl font-semibold text-white">{s.value.toLocaleString()}</div>
      ...
      <section id="browse" className="max-w-6xl mx-auto px-6 py-8">
        <div className="flex flex-col md:flex-row gap-3 mb-6">
          <div className="flex-1 relative">
            <input
              ...
              className="w-full bg-white/5 border border-white/10 rounded-md px-4 py-2.5 pl-10 text-sm focus:outline-none focus:border-white/30 transition-colors placeholder:text-neutral-500 text-white"
              value={search}
          ...
          <select
            className="bg-white/5 border border-white/10 rounded-md px-4 py-2.5 text-sm focus:outline-none focus:border-white/30 min-w-[200px] cursor-pointer text-neutral-300"
            value={selectedCategory}
            onChange={(e) => setSelectedCategory(e.target.value)}
          >
            {categories.map(([cat, count]) => (
              <option key={cat} value={cat} className="bg-[#0a0a0a]">{cat} ({count})</option>
            ))}
          </select>
          <div className="flex border border-white/10 rounded-md overflow-hidden">
            <button onClick={() => setViewMode("grid")} className={`px-3 py-2.5 ${viewMode === "grid" ? "bg-white text-black" : "bg-white/5 text-neutral-400 hover:text-white"}`}>
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zM14 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z" /></svg>
            </button>
            <button onClick={() => setViewMode("list")} className={`px-3 py-2.5 ${viewMode === "list" ? "bg-white text-black" : "bg-white/5 text-neutral-400 hover:text-white"}`}>
        ...
        <div className="flex items-center justify-between mb-6 text-sm">
          <span className="text-neutral-500">
            Showing <span className="text-white font-medium">{filteredLogos.length.toLocaleString()}</span> of {stats.brands.toLocaleString()}
          </span>
          {(search || selectedCategory !== "All") && (
            <button onClick={() => { setSearch(""); setSelectedCategory("All"); }} className="text-neutral-400 hover:text-white transition-colors">
              Clear filters
            </button>
          )}
        </div>
        ...
        {filteredLogos.length === 0 && (
          <div className="text-center py-24 border border-dashed border-white/10 rounded-lg">
            <p className="text-neutral-500 text-sm">No logos match your search.</p>
          </div>
        )}

        {/* Grid — DARK page, BRIGHT cards */}
            ...
            {filteredLogos.map(([key, brand]) => {
              ...
              return (
                <div
                  ...
                  className="group bg-[#111] rounded-lg border border-white/10 hover:border-white/30 transition-all cursor-pointer flex flex-col overflow-hidden"
                >
            ...
            {filteredLogos.map(([key, brand]) => {
              ...
              return (
                <div
                  key={key}
                  onClick={() => setSelectedBrand(key)}
                  className="flex items-center gap-4 bg-[#111] rounded-lg border border-white/10 hover:border-white/30 p-3 cursor-pointer transition-all"
                >
                  <div className="w-12 h-12 flex items-center justify-center bg-white rounded border border-neutral-200 shrink-0">
                    {!broken ? (
                      <img src={src} alt={brand.brand_name} loading="lazy" onError={() => handleImgError(src)} className="max-h-8 max-w-8 object-contain" />
                    ) : (
                      <span className="text-neutral-300 text-xs">—</span>
                    )}
                  </div>
      ...
      {selectedBrandData && selectedBrand && (
        <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4" onClick={() => setSelectedBrand(null)}>
          <div className="bg-[#111] border border-white/10 rounded-lg max-w-lg w-full max-h-[85vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <div className="p-6">
              <div className="flex items-start justify-between mb-4">
                <div>
                  <h2 className="text-xl font-semibold text-white">{selectedBrandData.brand_name}</h2>
                  <span className="inline-block mt-1 text-xs bg-white/10 text-neutral-300 px-2 py-0.5 rounded border border-white/10">
              ...
              <div className="flex flex-wrap gap-1.5 mb-5">
                {selectedBrandData.tags.map((tag) => (
                  <span key={tag} className="text-[10px] bg-white/5 text-neutral-400 px-2 py-0.5 rounded-full border border-white/5">{tag}</span>
                ))}
              </div>

              <div className="space-y-2">
                <h3 className="text-xs font-semibold text-neutral-500 uppercase tracking-wider mb-2">Assets</h3>
              ...
              <div className="mt-4 p-3 rounded bg-white/5 border border-white/10 text-[11px] text-neutral-400">
